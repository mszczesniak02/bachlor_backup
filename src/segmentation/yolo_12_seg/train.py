...
RESUME_MODEL_PATH = r"/content/old/runs/segment/yolov12m_crack_seg/weights/last.pt"


def train_model(epochs=YOLO_EPOCHS, batch_size=YOLO_BATCH_SIZE, lr=YOLO_LEARNING_RATE, model_size=YOLO_MODEL_SIZE):
    """
    Main training wrapper for YOLOv12
    """
    # Initialize model
    resume_flag = False
    if RESUME_MODEL_PATH and os.path.exists(RESUME_MODEL_PATH):
        ...
        model_name = f"yolo12{model_size}-seg.pt"  # e.g., yolo12n-seg.pt
        yaml_name = f"yolo12{model_size}-seg.yaml"

        if os.path.exists(model_name):
            print(f"Initializing YOLOv12 model from weights: {model_name}")
            model = YOLO(model_name)
        else:
            print(f"Weights {model_name} not found.")
            # Check for base config
            base_yaml = os.path.join(
                os.path.dirname(__file__), 'yolo12-seg.yaml')
            ...
            if os.path.exists(base_yaml):
                ...
                print(f"Initializing YOLOv12 model from config: {target_yaml}")
                model = YOLO(target_yaml)
                # Note: This initializes with random weights!
            else:
                # Fallback to trying to download or crash
                print(
                    f"Initializing YOLOv12 model: {model_name} (hoping for download)")
                model = YOLO(model_name)
    ...
    log_dir = UNET_MODEL_TRAIN_LOG_DIR + \
        str(datetime.now().strftime('%Y.%m.%d.%H_%M')) + "_yolo12"
    ...
    def on_fit_epoch_end(trainer):
        ...
        # YOLOv12 Seg: box_loss, seg_loss, cls_loss
    ...
    # Train
    # YOLOv12 handles logging and saving automatically to 'runs/segment/train'
    results = model.train(
        data=data_yaml,
        epochs=epochs,
        batch=batch_size,
        ...
        name=f"yolov12{model_size}_crack_seg",
